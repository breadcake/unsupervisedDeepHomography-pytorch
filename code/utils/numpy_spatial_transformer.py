...
def _interpolate(im, x, y, out_size, scale_H = SCALE_H):
    ...
    if im.ndim == 3:
      ...
      wa = np.tile(wa.reshape(-1, 1), num_channels)
      wb = np.tile(wb.reshape(-1, 1), num_channels)
      wc = np.tile(wc.reshape(-1, 1), num_channels)
      wd = np.tile(wd.reshape(-1, 1), num_channels)
...
def test_transformer(scale_H = SCALE_H): 
    img = io.imread('D:/Workspace/Datasets/ms_coco_test_images/COCO_test2014_000000000001.jpg')
    h, w = img.shape[0], img.shape[1]
    print( '-- h, w:', h, w ) 
